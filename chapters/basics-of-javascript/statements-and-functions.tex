...
JS의 equality comparison은 여타 언어들과 유사하게 \cd{==}과 \cd{!=} 연산자로 수행될 수 있으며, 비교 연산의 결과가 참이면 \cd{true}, 거짓이면 \cd{false}를 반환한다. 그러나 JS에서는 \cd{==}이나 \cd{!=} 연산자로 비교 연산을 수행하였을 때 문제가 발생할 수 있다.

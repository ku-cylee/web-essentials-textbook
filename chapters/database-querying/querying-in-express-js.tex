...
\coderef{code:runquery-prepared-statement}\는 \cd{runQuery} 함수를 수정한 코드로, 인자로 prepared statement(\cd{pstmt})와 \cd{?} 문자 자리에 들어갈 값들이 순서대로 할당된 배열(\cd{data})을 전달받아 SQL문을 생성(\cd{sql})한 뒤 DB에 전달하여 데이터를 전달받는다.

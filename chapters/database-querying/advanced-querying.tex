...
\cd{LIMIT} 키워드를 이용하면 조회 결과 중 \cd{offset}번째 row부터 \cd{row\_count}개의 row만 조회할 수 있으며, 이를 \cd{LIMIT <offset>, <row\_count>}의 형태로 작성한다. 이때 \cd{offset}은 0부터 count하고, 기본값은 0이다. \shellref{shell:limit-example}\은 \cd{scores} 테이블에서 성적의 내림차순으로 정렬한 row 중 2번째 row부터 4개의 row를 조회하는 예제이다.

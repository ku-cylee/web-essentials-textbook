...
\sqlref{sql:join}\은 \cd{left-table} 테이블에 \cd{right-table} 테이블을 \cd{join-condition}을 만족하며 join\footnote{MariaDB(MySQL)에서 full outer join 연산은 각 테이블에서 데이터를 조회한 뒤 합집합을 생성하여야 한다. 그러나 full outer join은 자주 쓰이는 연산이 아니므로 본 교재에서는 생략한다.}한 테이블을 생성하고, 그 테이블에서 \cd{SELECT} 키워드를 이용해 데이터를 조회하는 SQL문이다.

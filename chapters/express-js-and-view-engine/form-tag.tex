...
\subsection*{Sending POST Request From Browser}

지금까지는 Insomnia를 이용하여 웹 서버에 POST 요청을 보냈다. 그러나 대부분의 사용자는 이러한 프로그램을 사용하지 않고, 인터넷 브라우저에 띄워진 웹 페이지에서 요청을 보내야 한다. 그렇기 때문에 HTML 문서에서 \cd{body} 데이터와 함께 POST 요청을 보낼 수 있어야 한다.

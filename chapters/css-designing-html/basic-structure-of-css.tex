...
마지막 방법은 \textbf{external style sheet}으로, HTML 문서와 CSS 문서를 서로 다른 파일에 작성하고, HTML 문서의 \cd{head} 태그 내부에 \cd{link} 태그를 이용하여 CSS 파일을 연동한다. \cd{link} 태그에는 다음과 같은 속성을 지정해주어야 한다.

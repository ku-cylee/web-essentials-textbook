...
\figures{fig:css-basic-struct}{Basic Structure of CSS}
    {\fig{images/css-designing-html/css-basic-struct.png}{.7}}

CSS의 구조는 \figref{fig:css-basic-struct}\와 같이 표현될 수 있다. \textbf{선택자(selector)}는 HTML 요소를 태그 이름, 클래스 이름, 아이디, 상태, 속성 등을 기준으로 선택하는 방법을 서술하는 문자열이며, 선택자에 의해 선택된 요소들에 일괄적으로 적용할 스타일을 중괄호(\texttt{\{\}}) 내부에 작성한다.
...
마지막 방법은 \textbf{external style sheet}으로, HTML 문서와 CSS 문서를 서로 다른 파일에 작성하고, HTML 문서의 \texttt{head} 태그 내부에 \texttt{link} 태그를 이용하여 CSS 파일을 연동한다. \texttt{link} 태그에는 다음과 같은 속성을 지정해주어야 한다.

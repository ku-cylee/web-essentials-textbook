...
\figures{fig:css-basic-struct}{Basic Structure of CSS}
    {\fig{images/css-designing-html/css-basic-struct.png}{.55}}

CSS의 구조는 \figref{fig:css-basic-struct}\와 같이 표현될 수 있다. 선택자(selector)는 HTML 요소를 태그 이름, 클래스 이름, 아이디, 상태, 속성 등을 기준으로 HTML 요소를 선택하는 방법을 서술하는 문자열로, 작성 방법은 \sectref{sect:selectors}에서 구체적으로 다룬다. 선택자에 의해 선택된 요소들에 일괄적으로 적용할 스타일을 중괄호(\texttt{{}}) 내부에 작성한다.

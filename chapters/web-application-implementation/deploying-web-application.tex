...
\shellref{shell:project-config}\와 같이 git을 이용해 프로젝트를 clone하고, package.json에서 \cd{dependencies} 항목의 패키지를 설치한 뒤, schema/0000.sql 파일을 DB에 import한다. 이푸 Vim을 이용하여 .env 파일을 생성하는데, \cd{MODE}의 값을 \cd{prod}로 설정한다.
...
\shellref{shell:get-server-ip}\와 같이 서버 컴퓨터의 공용(public) IP 주소를 알아낸다.

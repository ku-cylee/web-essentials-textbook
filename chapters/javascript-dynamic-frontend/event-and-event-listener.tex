...
마지막 방법은 요소의 \texttt{addEventListener} 메서드를 사용하여 이벤트 리스너를 추가하는 방법으로, \texttt{addEventListener} 메서드는 이벤트의 이름과 이벤트 리스너를 인자로 받아, 이벤트 리스너를 추가하는 메서드이다. 이렇게 \texttt{addEventListener} 메서드를 이용한 방법이 가장 권장되는 방법이다.
...
\begin{itemize}
    ...
    \item \texttt{code}: 키 입력과 관련된 이벤트를 발생시킨 키를 문자열\footnote{https://keycode.info/에서 확인할 수 있다.}로 나타낸 속성
\end{itemize}

\coderef{code:event-example}\은 이벤트 객체를 이용하여 \texttt{\#panel} 요소를 좌클릭 또는 우클릭하였을 때 \texttt{\#panel} 요소의 위쪽 테두리와 왼쪽 테두리에 내린 수선을 표시되며, 좌클릭하였을 때는 빨간색, 우클릭하였을 때는 파란색 수선이 표시되는 예제이다. (편의상 CSS와 JS를 모두 HTML 문서에 작성하였다.)

...
\shellref{shell:description-of-table}\과 같이 \cd{DESC} 키워드를 사용하여 테이블의 설계 정보를 확인할 수 있습니다.
...
\shellref{shell:table-truncate-drop}\과 같이 테이블을 삭제할 수 있습니다. \cd{TRUNCATE} 키워드는 테이블의 구조는 유지한 채 데이터만 삭제한다면, \cd{DROP TABLE} 키워드는 테이블 자체를 삭제한다는 차이점이 있습니다.

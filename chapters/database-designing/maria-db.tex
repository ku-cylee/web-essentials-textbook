...
이제 각 OS의 shell을 통해 MariaDB에 접속할 수 있습니다. \shellref{shell:mariadb-login}\은 MariaDB에 로그인하는 가장 기본적인 명령어입니다. 지금은 root 계정만 생성한 상태이므로 root 계정으로 로그인합니다. \cd{<username>} 부분을 root으로, \cd{<password>} 부분을 root 계정의 비밀번호로 바꾸어 명령어를 작성하면 됩니다.
...
유닉스 계열의 OS에서 \shellref{shell:mariadb-login}의 방법대로 root 계정에 접속하지 못하는 경우가 있습니다. 이런 경우 \shellref{shell:mariadb-sudo-login}\과 같이 sudo 권한으로 실행하면 자동으로 root 계정으로 접속됩니다.
...
\shellref{shell:query-db-list}\와 같이 SQL을 사용하여 현재 DBMS에 있는 데이터베이스의 목록을 조회할 수 있습니다. 커서 부분에서 대괄호(\cd{[]}) 내부는 현재 사용자가 작업 중인 데이터베이스의 이름을 나타냅니다. 현재는 작업 중인 데이터베이스가 없으므로 \cd{(none)}으로 표시됩니다.
...
\shellref{shell:create-and-use-db}\와 같이 \cd{kweb\_db}라는 데이터베이스를 생성하고, 작업 중인 데이터베이스를 \cd{kweb\_db}로 바꿀 수 있습니다.

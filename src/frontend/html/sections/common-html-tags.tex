\section{Commonly Used HTML Tags}\label{sect:common-html-tags}
...
\begin{code}{code:heading-tags}{\cd{h1} - \cd{h6} tags}
\begin{minted}{html}
...
\begin{code}{code:par-tag}{\cd{p}, \cd{br} tags}
\begin{minted}{html}
<p>
...
\begin{code}{code:button-tag}{\cd{button} tag}
\begin{minted}{html}
...
\begin{code}{code:img-tag-url}
    {\cd{img} tag\protect\footnotemark for online image}
\begin{minted}{html}
...
\end{minted}
\end{code}
\footnotetext{이 코드에서는 HTML의 속성-속성값 쌍을 분리하기 위해 1칸의 공백 대신 개행 문자와 5칸의 공백을 사용하였다. HTML에서는 연속된 whitespace 문자들을 하나의 공백으로 취급하기 때문에 이렇게 표시할 수 있으며, 속성이 많거나, 속성값이 길거나, 내용이 길거나 등의 이유로 HTML 요소가 지나치게 길어지면 이와 같이 개행하여 작성하는 것이 좋다.}
...
\begin{code}{code:img-tag-local}{\cd{img} tag for local image}
\begin{minted}{html}
...
\begin{code}{code:nbsp-usage}{Usage of \cd{\&nbsp;}}
\begin{minted}{html}
...
\begin{code}{code:div-and-span-tags}{\cd{div} and \cd{span} tags}
\begin{minted}{html}
<div>
    <h3>&lt;div&gt; tag</h3>
    <div>

...
\cd{button} 태그는 클릭할 수 있는 버튼을 생성하는 태그이다. 이 태그를 눌렀다가 놓았을 때 특정한 동작이 수행\footnote{\chapref{ch:javascript-dynamic-frontend}에서 다룸}될 수 있으며, \cd{input} 태그를 이용해서도 유사한 기능을 구현할 수는 있으나 요소를 디자인할 때 \cd{button} 태그가 더 편하므로 더 많이 사용된다.

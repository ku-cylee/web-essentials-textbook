...
\begin{itemize}
    ...
    \item \cd{substring(indexStart[, indexEnd]) => string}: 문자열의 일부분을 추출하는 메서드; \cd{indexStart}번째 문자부터 \cd{indexEnd}번째 문자 직전까지의 문자열을 추출하여 반환하며, \cd{indexEnd}의 기본값은 \cd{length} 값이다.

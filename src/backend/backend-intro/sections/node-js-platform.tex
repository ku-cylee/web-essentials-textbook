...
Windows와 macOS에서는 홈페이지에서 LTS 버전 설치 파일을 다운로드한 후, 설치 파일을 실행해 추가 옵션 없이 설치한다. Linux 계열 운영체제는 Terminal\footnote{Shell에서 \cd{\$} 문자는 입력란을 뜻하므로 \cd{\$ } 부분은 입력하지 않는다.}을 열고 \shellref{shell:nodejs-linux-install}\을 따라 설치 파일을 다운로드한 후 설치한다. 이때 \cd{<major-version>} 부분에 다운로드 하고자 하는 버전의 major 번호\footnote{16.17.0 버전의 경우 16}를 대입한다.
